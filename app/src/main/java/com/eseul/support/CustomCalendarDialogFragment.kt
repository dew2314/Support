...
class CustomCalendarDialogFragment(private val onDateSelected: (String) -> Unit) :
    DialogFragment() {
    ...
    override fun onCreateView(
        ...
        val rootView = inflater.inflate(R.layout.dialog_calendar, container, false)
        setupCustomCalendar(rootView)

        // 확인 버튼 설정
        val confirmButton: TextView = rootView.findViewById(R.id.confirm_button)
        confirmButton.setOnClickListener {
            onDateSelected(dateText.text.toString())
            dismiss()
        }
    ...
    override fun onResume() {
        ...
        dialog?.window?.let { window ->
            ...
            // 화면 너비에서 여백(24dp * 2)을 제외한 다이얼로그 너비 계산
            val marginInDp = 15
            val marginTopInDp = 130
            val width =
                (displayMetrics.widthPixels - (marginInDp * 2 * displayMetrics.density)).toInt()
            val height = (390 * displayMetrics.density).toInt() // 고정된 높이

            // 다이얼로그 크기 설정
            window.setLayout(width, height)

            // 다이얼로그 위치 중앙으로 설정하면서 상단에서 160dp 떨어지게 설정
            val params = window.attributes
            params.gravity = Gravity.TOP or Gravity.CENTER_HORIZONTAL
            params.y = (marginTopInDp * displayMetrics.density).toInt() // marginTop 설정
